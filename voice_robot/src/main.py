...
def publish_nav_goal(destination: str) -> None:
    if not USE_ROS2:
        log.info(f"[ROS2 stub] Navigating to: {destination}")
        return
    try:
        import rclpy
        ...
        node = rclpy.create_node("voice_nav_publisher")
        pub = node.create_publisher(String, ROS2_TOPIC, 10)
        time.sleep(0.2)
        msg = String()
        msg.data = destination
        pub.publish(msg)
        log.info(f"[ROS2] Published: {destination}")
        node.destroy_node()
        rclpy.shutdown()
    except Exception as e:
        log.error(f"ROS2 publish failed: {e}")
...
def main():
    ...
    while True:
        ...
        publish_nav_goal(dest_confirmed)
        ...
        if AUTO_RETURN_HOME and current != HOME_LOCATION:
            ...
            if return_path:
                publish_nav_goal(HOME_LOCATION)
                time.sleep(2)
                current = HOME_LOCATION
